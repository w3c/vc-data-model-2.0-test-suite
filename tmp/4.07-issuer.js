    ...
    describe(name, function() {
      ...
      it('A verifiable credential MUST have an issuer property.',
        async function() {
          ...
          const vc = await endpoints.issue(
            require('../tests/input/credential-ok.json'));
      ...
      async function() {
        ...
        await assert.doesNotReject(endpoints.issue(require(
          '../tests/input/credential-issuer-object-ok.json')));
        await assert.rejects(endpoints.issue(require(
          '../tests/input/credential-issuer-no-url-fail.json')),

        'Failed to reject an issuer identifier that was not a URL.');
        await assert.rejects(endpoints.issue(require(
          '../tests/input/credential-issuer-null-fail.json')),

        'Failed to reject a null issuer identifier.');
        await assert.rejects(endpoints.issue(require(
          '../tests/input/credential-issuer-object-id-null-fail.json')),

        'Failed to reject an issuer object containing a null identifier.');
        await assert.rejects(endpoints.issue(require(
          '../tests/input/credential-issuer-object-id-no-url-fail.json')),
