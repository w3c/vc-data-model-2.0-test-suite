...
import {shouldBeCredential} from '../tests/assertions.js';
      ...
      async function() {
        ...
        if(!vpVerifierEnvelopeSupport) {
          ...
          await assert.doesNotReject(endpoints.verifyVp(require(
            '../tests/input/presentation-enveloped-vc-ok.json')),
          ...
          await assert.rejects(endpoints.verifyVp(require(
            '../tests/input/presentation-enveloped-vc-missing-type-fail.json')),
    ...
    describe(name, function() {
      ...
      it('The type value of the object MUST be ' +
        ...
        if(!vpVerifierEnvelopeSupport) {
          ...
          await assert.rejects(
            endpoints.verifyVp(require(
              '../tests/input/enveloped-presentation-type-fail.json')),
